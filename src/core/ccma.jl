...
function _filter(ccma::CCMA, points, w_ma, w_cc, cc_mode)
    ...
    # println(size(points, 1) - 2 * w_ccma)
...
function filter(ccma::CCMA, points::Matrix{Float64}; mode="padding", cc_mode=true)
    ...
    if mode != "fill_boundary"
        ...
        if cc_mode
            ...
            for (idx_w, width_set) in enumerate(descending_width_list)
                w_ccma = width_set["w_ma"] + width_set["w_cc"] + 1
                use_ma_1 = width_set["w_ma"] == 0 && ccma.w_ma != 0
                
                # print(size(points_ccma))
                points_ccma[idx_w + 1, :] = _filter(
                    ccma, 
                    points[1:idx_w + 1 + w_ccma, :],
                    !use_ma_1 ? width_set["w_ma"] : 1,
                    width_set["w_cc"],
                    !use_ma_1)[:, 1:dim]
                ...
                points_ccma[end-idx_w - 2 + 1 + 1, :] = _filter(
                    ccma, 
                    points[end - (idx_w + 2 + w_ccma) + 2:end, :],
